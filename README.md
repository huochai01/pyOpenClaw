...
`mini-openclaw` 的目标不是做一个重型 SaaS 平台，而是提供一套透明、可控、易修改的本地 Agent 系统：会话、记忆、技能、知识库、定时任务都尽量落在本地文件系统中，便于查看、编辑、调试和扩展。
...
- 修改技能时，优先改 `SKILL.md`
- 修改系统行为时，优先改 `backend/workspace/` 下的 Markdown 文件
- 修改持久化逻辑时，重点关注：
  - `backend/sessions/`
  - `backend/memory/`
  - `backend/storage/`
...
- 定时任务当前只支持“每天固定时间”
- 当前更适合本地单用户或实验型场景，不是面向高并发生产环境

## License

当前仓库未单独声明许可证。  
如果你计划公开发布，建议补充 `LICENSE` 文件。
